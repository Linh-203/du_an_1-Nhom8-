    ...
    //   unset( $_SESSION["cart"]);
      ...
      if(isset($_POST["submit"])){
        if(empty($_POST["search"])){
            ...
            $search = $_POST["search"];
            $query = "SELECT * FROM oder where id_user like n'$id' and id = $search";
            $oder = getAll($query);
        }
      }
...
<body>
    ...
    <header>
        ...
        <form action="" style="background-color: ;border-radius: 7px;display: flex;margin-top: 20px;" method="POST">
          <input name="search" type="text" placeholder="Tìm kiếm hóa đơn theo mã..." style="width: 300px;background-color: #eaeaea;padding: 10px;border: none;outline: none;">
          <button name="submit" type="submit"><i style="font-size: 20px;border-radius: 0 7px 7px 0;background-color: rgba(243, 168, 71, 1);height: 40px; padding:10px;text-align: center; " class="fa fa-search"></i></button>
        </form>
